const modules = [
  ...
  },
  {
    ...
    label: "数据合规",
    subtitle: "系统与专项治理",
    description: "跟踪系统治理、专项整改与资讯看板建设，区分进度表和总表。",
  },
  {
    id: "dispute",
    label: "争议与人事风险",
    subtitle: "诉讼劳动信安案件",
    description: "按诉讼、劳动人事、信安拆分展示进度表与总表，支持点击标题查看详情。",
  },
  {
    id: "investment",
    label: "战投与公司治理",
    subtitle: "投资项目与主体治理",
    description: "跟踪战略投资项目、非项目事项及公司治理事项，展示项目类型、区域、上周与本周进展。",
  },
  {
    id: "ip",
    label: "IP资产与保护",
  ...
  },
  {
    ...
    label: "运营与职能支持",
    subtitle: "AI系统流程职能",
    description: "拆分重点推进与日常维护，覆盖 AI 能力、法务系统、职能支持、流程沉淀及其他事项。",
  },
];
...
const highlights = [
  {
    ...
    group: "项目支持",
    risk: "中",
    text: "围绕 DM01、RM42 等项目输出协议、发行、商业化规则和客诉处理意见。",
  },
  {
    title: "数据合规整改推进",
    group: "数据合规",
    risk: "高",
    text: "账号、客服等系统的个人信息导出、存储周期、权限审批问题形成初步整改方案。",
  },
  {
    title: "IP资产保护持续推进",
    group: "IP资产与保护",
    risk: "低",
    text: "专利查新、商标复审、著作权登记和平台投诉下架事项均有当周进展。",
  },
  {
    title: "运营体系建设",
    group: "运营与组织能力",
    risk: "低",
    text: "法务 AI、合同系统、预算和工作台需求进入持续建设与确认阶段。",
  },
];
...
const moduleItems = {
  ...
  data: [
    ...
    {
      name: "数据合规资讯可视化",
      result: "持续更新数据合规资讯页面，增强一页纸看板的可读性。",
      background: "合规资讯页面与一页纸看板建设。",
      progress: "持续更新数据合规资讯页面，增强一页纸看板的可读性。",
      status: "推进中",
      risk: "低",
      project: "合规看板",
      source: "数据组/法务运营",
...
function groupTone(group) {
  if (group.includes("数据")) return "group-data";
  if (group.includes("IP")) return "group-ip";
  if (group.includes("争议") || group.includes("人事")) return "group-dispute";
...
function renderHighlights() {
  document.querySelector("#highlightGrid").innerHTML = highlights
    .map(
      (highlight) => `
        <article class="highlight-row compact-record">
          <strong>${highlight.title}</strong>
          <div class="tag-row">
            <span class="tag ${groupTone(highlight.group)}">${highlight.group}</span>
            <span class="tag ${riskTone(highlight.risk)}">${riskLabel(highlight.risk)}</span>
          </div>
          <p><span class="content-label">概况</span>${highlight.text}</p>
        </article>
      `,
    )
    .join("");
...
function renderOpsComboChart(item) {
  ...
  const colors = ["#f59e0b", "#f97316", "#facc15", "#d97706", "#fb923c", "#fbbf24"];
          ...
          <div class="ops-top">
            <div>
              <div class="ops-title-line">
                ...
                ${
                  item.metrics
                    ? `<span class="tag ${item.current > 0 ? "amber" : "green"}">${item.status}</span>`
                    : `<button class="tag ops-new-link ${item.current > 0 ? "amber" : "green"}" type="button" data-ops-new-id="${item.id}">${item.status}</button>`
                }
              </div>
            </div>
            ${
...
function projectScopeClass(scope) {
  if (scope === "数据合规") return "project-scope-data";
...
const disputeSections = [
  ...
  },
  {
    ...
    label: "劳动人事",
    className: "labor",
    intro: "劳动人事相关项目按标签管理，覆盖 ER、竞业限制、招聘、OE、HRBP 等事项。",
  },
  {
    type: "security",
    label: "信安",
    className: "security",
    intro: "信安相关人员事项独立跟踪，覆盖定级、证据留存与后续处置。",
  },
];
...
function renderDisputeRiskDashboard(items) {
  return `
            ...
            <article class="dispute-section dispute-section--${section.className}">
              ...
              <div class="dispute-section-body">
                <div class="dispute-left-stack">
                  <section class="dispute-subcard dispute-progress-card">
                    <div class="dispute-subhead">
                      <h4>进度表</h4>
                      <span>${
                        section.type === "labor"
                ...
                <section class="dispute-subcard dispute-history-card">
                  <div class="dispute-subhead">
...
function renderInvestmentDashboard(items) {
      ...
      <article class="investment-section strategy-section">
        ...
        <div class="investment-workspace">
          <div class="investment-left-stack">
            <section class="investment-subcard progress-register investment-progress-panel">
            ...
            <section class="investment-subcard investment-other-register">
              <div class="investment-subhead">
                <h4>其他事项</h4>
            ...
            <section class="investment-subcard project-register investment-project-register-panel">
              <div class="investment-subhead">
                <div class="table-head-copy">
                  <h4>总表</h4>
                  <span class="timeline-hint" aria-label="点击标题可查看详情"><span aria-hidden="true">💡</span>点击标题可查看详情</span>
          ...
          ${
            activeItems.length
              ? activeItems
                  ...
                  .join("")
              : `<div class="empty-group">暂无推进中的数据合规专项。</div>`
          }
        </div>
      </article>
...
function renderOperationsSupportDashboard(items) {
  const allItems = [...items, ...(moduleItems.support || [])];
  ...
  const fieldHeader = (modifier, fields) => `
            ...
            <div class="ops-simple-list">
              ${
                knowledgeItems.length
                  ? `
                      ${fieldHeader("knowledge", ["流程名称", "沉淀内容", "适用场景"])}
                      ${knowledgeItems
                      .map(
                        (item) => `
            ...
            <div class="ops-simple-list">
              ${
                functionItems.length
                  ? `
                      ${fieldHeader("function", ["事项", "本周处理", "待确认/风险"])}
                      ${functionItems
                      .map(
                        (item) => `
...
function renderModuleView(moduleId) {
  ...
  const items = moduleItems[moduleId] || [];
  ...
  if (moduleId === "investment") {
    ...
    itemTable.innerHTML = renderInvestmentDashboard(items);
    return;
  }
  ...
  if (moduleId === "dispute") {
    ...
    itemTable.innerHTML = renderDisputeRiskDashboard(items);
    return;
  }
